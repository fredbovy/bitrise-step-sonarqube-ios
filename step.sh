...
# SonarQube
sonarScannerOptions="-Dsonar.host.url=${sonar_host_url} -Dsonar.login=${SONAR_HOST_LOGIN} -Dsonar.projectKey=${project_key} -Dsonar.organization=${sonar_host_organization} -Dsonar.language=swift -Dsonar.exclusions=${exclusions} -Dsonar.flex.cobertura.reportPaths=./sonar-reports/coverage-swift.xml"
